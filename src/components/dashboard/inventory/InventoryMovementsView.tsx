...
export function InventoryMovementsView({
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredMovements.map((m) => [
      ...
      m.previousStock,
      m.newStock,
      `"${(m.createdBy?.name || 'System').replace(/"/g, '""')}"`,
      m.referenceId || '',
      `"${(m.notes || '').replace(/"/g, '""')}"`,
    ]);
    ...
    link.setAttribute(
      'download',
      `inventory_movements_${new Date().toISOString().slice(0, 10)}.csv`,
    );
    ...
    <div className="h-full flex-1 flex flex-col min-h-0 bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-100 overflow-hidden font-sans">
      ...
      <div className="px-6 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-4 bg-white dark:bg-slate-900 shrink-0">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-1.5 bg-[#e6f6f3] hover:bg-[#d6f0ea] text-[#0d5c56] dark:bg-[#0d6157]/25 dark:hover:bg-[#0d6157]/40 dark:text-teal-300 font-semibold text-xs px-3.5 py-1.5 rounded-[8px] border border-[#0d8276]/20 transition-all cursor-pointer shrink-0 disabled:opacity-50"
        >
          ...
          <table className="w-full text-left text-xs border-collapse min-w-[900px]">
            <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200 dark:border-slate-800 z-10">
              <tr>
                ...
                <th className="py-2.5 px-6 whitespace-nowrap">Reference / Reason</th>
              </tr>
            </thead>
                    ...
                    >
                      ...
                      <td className="py-3 px-4 font-medium text-slate-900 dark:text-white whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          ...
                          {m.item.sku && (
                            <span className="text-[10px] text-slate-400 font-mono bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-[4px]">
                              {m.item.sku}
                            </span>
                          )}
                        </div>
                      ...
                      <td className="py-3 px-6 text-slate-600 dark:text-slate-400 text-xs">
                        <div className="flex items-center gap-2 flex-wrap">
                          {m.referenceId && (
                            <span className="font-mono font-bold text-[10px] text-[#0d5c56] dark:text-teal-300 bg-[#e6f6f3] dark:bg-[#0d6157]/20 px-2 py-0.5 rounded-[6px] border border-[#0d8276]/20 whitespace-nowrap shrink-0">
                              #{m.referenceId}
                            </span>
                          )}
